...
use crate::{
    ...
    execution::{
        ...
        resolve_execution_settings, resolve_server_binding, validate_non_execution_command_flags,
...
fn profile_flag_is_rejected() {
    let error = Args::try_parse_from(["build-eips", "--profile", "local", "build"]).unwrap_err();
...
fn explicit_workspace_config_path_is_not_accepted() {
    let error =
        Args::try_parse_from(["build-eips", "--config", "/tmp/config.toml", "build"]).unwrap_err();
...
fn local_first_without_workspace_config_requires_explicit_resolution() {
    ...
    assert!(message.contains("selected command requires workspace-local theme and sibling sources"));
    ...
    assert!(message
        .contains("use `parity <command>`, `--staging <command>`, or `--production <command>`"));
